...
# ИЗМЕНЕНО: Сразу прописана нужная вам версия v0.2.5, чтобы не использовать sed
...
install_awg_packages() {
    ...
    # ИЗМЕНЕНО: Проверка и установка luci-proto-amneziawg вместо luci-app-amneziawg
    if opkg list-installed | grep -q luci-proto-amneziawg; then
        echo "luci-proto-amneziawg already installed"
    else
        # ИЗМЕНЕНО: имя файла изменено с app на proto
